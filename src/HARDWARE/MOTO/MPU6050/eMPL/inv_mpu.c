...
#define MPU6050							//定義我們使用的感測器為MPU6050
...
//}
#define log_i 	printf	//列印資訊
#define log_e  	printf	//列印資訊
...
static int get_accel_prod_shift(float *st_shift)
{
    ...
    //  return 0x07;
    for(int r=0; r<4; r++) {
      tmp[r] = MPU_Read_Byte(MPU_SELF_TESTX_REG+r); // 0X0D
    }
...
static int accel_self_test(long *bias_regular, long *bias_st)
{
  ...
  //MPU_Write_Byte(MPU_ACCEL_CFG_REG, 0xF0);
  uint8_t c = MPU_Read_Byte(MPU_ACCEL_CFG_REG);
  printf("accel_self_test: cfg=%x\n", c);
...
static int gyro_self_test(long *bias_regular, long *bias_st)
{
  ...
  //MPU_Write_Byte(MPU_GYRO_CFG_REG, 0xF0);
  uint8_t c = MPU_Read_Byte(MPU_GYRO_CFG_REG);
  printf("gyro_self_test: cfg=%x\n", c);
...
#endif
restore:
printf("T05: gy:%x ac:%x\n", gyro_result, accel_result);
#elif defined MPU6500
...
//	MPU_IIC_Init(); 	//初始化IIC匯流排
	...
	{	 
		res=mpu_set_sensors(INV_XYZ_GYRO|INV_XYZ_ACCEL);//設定所需要的感測器
		if(res)return 1;

		res=mpu_configure_fifo(INV_XYZ_GYRO|INV_XYZ_ACCEL);//設定FIFO
		if(res)return 2;

		res=mpu_set_sample_rate(DEFAULT_MPU_HZ);	//設定採樣率
		if(res)return 3; 

		res=dmp_load_motion_driver_firmware();		//載入dmp韌體
		if(res)return 4; 
...
//得到dmp處理后的數據(注意,本函式需要比較多堆疊,區域性變數有點多)
